    ...
    <div style={styles.container}>
      <div style={styles.navbar}>
        ...
        <div style={styles.navRight}>
          ...
          <Link to="/add-report" style={styles.addBtn}>+ Tambah Laporan</Link>
          <button style={styles.logoutBtn} onClick={handleLogout}>Logout</button>
        </div>
      </div>

      <div style={styles.content}>
        <h3 style={styles.sectionTitle}>Daftar Laporan</h3>
        {reports.length === 0 ? (
          ...
          reports.map((report) => (
            <div key={report.id} style={styles.card}>
              ...
              <div style={styles.cardFooter}>
                <span style={styles.meta}>Oleh: {report.username} | {report.category_name}</span>
                <Link to={`/report/${report.id}`} style={styles.detailBtn}>Lihat Detail</Link>
              </div>
            </div>
          ))
        )}
...
const styles: { [key: string]: React.CSSProperties } = {
  ...
  addBtn: { backgroundColor: 'white', color: '#f48fb1', padding: '8px 16px', borderRadius: '20px', textDecoration: 'none', fontSize: '14px', fontWeight: '600' },
  ...
  emptyContainer: { textAlign: 'center', padding: '3rem', backgroundColor: 'white', borderRadius: '16px', boxShadow: '0 2px 10px rgba(0,0,0,0.05)' },
  ...
  cardBody: { color: '#888', fontSize: '14px', marginBottom: '1rem', lineHeight: '1.6' },
  cardFooter: { display: 'flex', justifyContent: 'space-between', alignItems: 'center' },
  meta: { fontSize: '12px', color: '#aaa' },
  detailBtn: { backgroundColor: '#f48fb1', color: 'white', padding: '6px 16px', borderRadius: '20px', textDecoration: 'none', fontSize: '13px', fontWeight: '500' }
};
